...
def end_to_end(dat):
    ...
    if len(endpts) == 2:
        ...
        return dat.get_px_size()*np.max(lengths)

def wormlike_chain(cl,pl):
    return 4*pl*cl*(1-2*pl/cl*(1-np.exp(-cl/(2*pl))))

def Lp(data):
    cl = [contour(n) for n in data]
    eel2 = [end_to_end(n)**2 for n in data]
    popt, pcov = curve_fit(wormlike_chain,cl,eel2)
    return popt[0]
...
def plot(data):
    ...
    fig, ax = plt.subplots()
    ax.scatter(cl,np.sqrt(eel2))
    ax.plot(x,np.sqrt(y2))
